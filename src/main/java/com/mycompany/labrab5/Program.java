...
public static void getPos(player player) { System.out.print("Позиция игрока - " + player.ReturnPosition()); }
            ...
            while (check == 1)
            {
                ...
                if (action == 1)
                    ...
                    if (action == 12)
                    {
                     ...
                     if (action == 13)
                {
                    int k, i; k = a.return_yellow_cards();
                    for (i = 0; k >= field_player.RetDisq(); i++)
                    {
                        k -= field_player.RetDisq();
                    }
                    System.out.print("Количество матчей = " + i+"\n");
                }
                      if (action == 14)
                    {
                        getPos(a);
